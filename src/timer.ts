  ...
  const loop = () => {
    ...
    if (cancel) {
      if (appState() !== States.Timing) {
        timePreviouslyStretched(timePreviouslyStretched() + timedSoFar);
    ...
    keepstretching.textContent =
      timedSoFar > 1000 * stretchLength()
        ? `Don't push yourself too hard! You're ${formatTime(
            timedSoFar - 1000 * stretchLength()
          )} over your target`
        : `Keep going for now, take a break in ${formatTime(
            1000 * stretchLength() - timedSoFar
          )}...`;

    stretchgoal.textContent =
      timedSoFar + timePreviouslyStretched() > 1000 * fullLength()
        ? `Nice work! You've exceeded your total goal by ${formatTime(
            timedSoFar + timePreviouslyStretched() - 1000 * fullLength()
          )}`
        : `Keep it up, ${formatTime(
            1000 * fullLength() - (timedSoFar + timePreviouslyStretched())
          )} left in total`;

    goalremaining.textContent = formatTime(
      1000 * fullLength() - timePreviouslyStretched()
    );
